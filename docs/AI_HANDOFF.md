# ORCA AI HANDOFF

이 문서는 ORCA 프로젝트에서 FE와 BE/OCR 작업 내용을 서로 전달하기 위한 간단한 기준입니다.

## 브랜치 역할
- `main`: 안정판 + 공용 docs
- `develop`: FE와 BE/OCR/Supabase를 합쳐 계속 테스트하는 공용 개발판
- `fe-dev`: 주로 FE 작업
- `be-dev`: 주로 BE / OCR / Supabase 작업

브랜치를 지나치게 엄격하게 분리하지 않습니다.
각 브랜치에는 저장소 전체 파일이 있을 수 있고, 필요한 경우 연동 테스트를 위해 다른 영역 파일을 볼 수 있습니다.
별도 integration 브랜치는 사용하지 않습니다. 통합 테스트 역할은 `develop`이 담당합니다.

## 작업 로그
공용 로그의 기준본은 `main` 브랜치의 `docs/`입니다. 코드 개발 브랜치와 별개로 서로 전달할 내용은 이 로그에 기록합니다.

- FE 관련 작업/변경/요청: `docs/FE_LOG.md`
- BE/OCR/Supabase 관련 작업/변경/요청: `docs/BE_LOG.md`

작업을 시작할 때 자신의 영역 로그를 먼저 확인합니다.
상대 영역에서 알아야 할 내용이나 수정 요청이 생기면 해당 로그에 남깁니다.

## 로그에 남길 내용
복잡한 형식은 강제하지 않습니다.
- 날짜
- 상태: TODO / IN_PROGRESS / DONE / NOTE
- 내용
- 관련 파일 또는 기능
- 필요하면 완료 커밋

## 작업 흐름
- FE 작업 묶음 완료: `fe-dev → develop`
- BE/OCR/Supabase 작업 묶음 완료: `be-dev → develop`
- 다음 작업 시작 전에는 `develop`의 최신 통합 상태를 자기 브랜치에 반영합니다.
- `fe-dev ↔ be-dev` 직접 Merge는 피합니다.
- `develop`에서 통합 확인이 끝난 안정판만 `main`으로 올립니다.

## 기본 원칙
- 현재 방식으로 자유롭게 작업하되, 서로 충돌할 가능성이 있는 변경은 로그에 남깁니다.
- PC마다 달라질 수 있는 경로, 키, 주소는 가능하면 환경변수나 설정값으로 처리합니다.
- `.env.local`, 비밀키, 개인 토큰은 GitHub에 올리지 않습니다.
- 문제가 실제로 생기기 전까지 과도한 브랜치/파일 수정 제한은 두지 않습니다.

## AI 작업 진행 원칙
- 사용자가 다음 작업을 요청했거나 다음 단계가 명확하고 ChatGPT가 도구로 직접 수행할 수 있는 경우, 설명만 하고 기다리지 말고 같은 응답에서 바로 진행한다.
- 사용자 확인이 필요한 경우는 비가역적 변경, 권한/승인 필요 작업, 정보 부족으로 잘못된 변경 위험이 큰 경우에만 묻는다.
