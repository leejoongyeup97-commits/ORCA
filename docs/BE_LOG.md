...
### 2026-09-26 · 제안사항: OCR 서버 실행 안정성 / 준비 상태 점검
- 상태: PROPOSAL / BE 검토 후 작업
- 배경:
  - FE에서 OCR 요청 시 `http://127.0.0.1:8001`에 연결하지 못하는 상황이 반복 발생
  - FE 쪽에서는 사용자에게 진행률/오류 원문을 보여주는 UI만 담당하고, OCR 서버가 정상적으로 기동되고 `/health`가 응답하는 문제는 BE 영역으로 분리
- BE 확인 요청:
  - `START_OCR.bat` 실행 시 서버가 항상 정상 기동되는지
  - Python dependency 설치/동기화 실패 여부
  - Tesseract 실행 파일 및 `eng`/`kor` 언어팩 감지
  - 포트 8001 충돌/점유 처리
  - 서버 기동 완료 전 요청이 들어오는 경우의 상태 처리
  - `/health`가 실제 OCR 가능 상태를 정확히 반환하는지
- 참고:
  - FE에서는 연결 실패 시 오류 모달/원문 보기/복사 기능을 제공하도록 처리함.
  - 서버 자체 기동 실패나 준비 지연의 근본 원인은 BE에서 해결하는 방향 권장.
...
- IN_PROGRESS: Team production OCR 단독 API 테스트 성공. ocr_version 0.9.12-dev / hero_matching_status scoreboard_native_v1 / 숫자·is_me·hero_id 동시 반환 확인. 테스트 스크린샷(23-03-45)은 10/10 영웅과 K/A/D가 ground truth와 일치. 단, 현재 production reference는 35영웅/100 crop만 포함하므로 미수집 영웅과 singleton 영웅의 새 이미지 일반화는 추가 검증 필요.
- IN_PROGRESS: 영웅 reference 커버리지 53명 중 35명 확보, 18명 미확보, singleton 17명. 목표는 영웅당 scoreboard reference 최소 2개. 역할별 추가 캡처 필요 수량을 자동 계산하도록 REPORT_HERO_COVERAGE 확장 (be-dev: fa950584983db7ff38da1b135687bd9c840d559e).
- IN_PROGRESS: Pukima 커뮤니티 Hero Gallery/Scoreboard icon 세트는 현재 Team crop 벤치마크에서 18/75 = 24.0%로 부적합 판정. 대신 최신 OWLib/DataTool의 `extract-hero-icons` 모드로 로컬 Overwatch 설치에서 현재 GUI 영웅 이미지를 직접 추출하는 경로로 전환. be-dev에 `EXTRACT_OW_HERO_ICONS.bat` 추가, DataTool 바이너리와 추출물은 gitignore 처리.
- IN_PROGRESS: 6v6 Team 스크린샷은 production 5v5 OCR과 분리해 reference 수집용으로만 사용. `images_6v6` 전용 12-row hero crop extractor 및 montage/manifest 생성 도구 추가. be-dev commits: 6b551ef6640ca542277c63d6ff73fd947cf3294e, b0840662f6d51c6329da524d936a4189ae3a39ec, 7fed0a68650381248f49ca4ee98d76268b1e3140
- IN_PROGRESS: 6v6 reference 수집에서 6장 모두 12/12 row detection 성공(72 crops). 기존 crop에 역할 아이콘이 섞이는 문제를 발견해 hero-only x 범위로 수정. 기존 5v5 production reference로 6v6 crop을 고신뢰 자동 라벨링하는 도구 추가. be-dev commits: d8f6d214e93fd177958171aebac7514bf71e6dd4, 59082cd4b6582932921092cf8b412fe91441cabf, b59d4e34e13e992c2f2800c747612b9d5b470701
- NOTE: 6v6 hero auto-label 실험은 best-score median 0.210, margin median 0.171로 5v5 reference와 구도 차이가 커서 자동 라벨 0/72. 추가 공수 대비 효율이 낮아 6v6 reference 경로는 보류하고, 5v5 Team 캡처 추가 수집으로 전체 영웅 커버리지를 확장하는 방향으로 전환.
